...
1. **CNLSA (The Disease Model):** We characterize VAE-induced CLIP semantic drift, showing it is (a) substantial in magnitude (CLIP similarity drops from 1.0 to 0.9388 at σ=0), (b) category-uniform across semantic categories (ANOVA p=0.6037), (c) CLIP-specific — affecting CLIP ViT-B/16 more severely (CS=0.343) than CLIP ViT-S/14 (CS=0.8155), and (d) **not factor-separable** — we falsify the hypothesis that VAE encoding increases CLIP-DINOv2 correlation; the drift mechanism is unitary, not a combination of separable factors.
...
### 3.3 CLIP Semantic Drift at σ=0 (Encode–Decode Gate)

At σ=0 (pure VAE encode–decode roundtrip, no additional noise), CLIP ViT-B/16 cosine similarity drops to CS=0.343 — a devastating semantic distortion. In contrast, CLIP ViT-S/14 is far more robust: CS=0.8155. A key validation: **CLIP ViT-S/14 and DINOv2 ViT-S/14 produce near-identical scores across all σ values** (Pearson r≈0.99 between the two encoders' per-image similarity measurements). This close agreement validates DINOv2 as a CLIP proxy — DINOv2 ViT-S/14 tracks CLIP ViT-S/14's response to VAE perturbation almost perfectly. The unique vulnerability of CLIP ViT-B/16 at σ=0 (CS=0.343) — dropping far below both ViT-S variants — indicates that the disease mechanism is CLIP-specific but architecture-variant: it is not a general property of "large models" but something specific to the CLIP ViT-B/16 architecture at identity noise. This CLIP-specificity supports the CNLSA framing: the disease targets the CLIP–VAE interaction, not all semantic embeddings universally.
...
### 3.5 Interpretation: CLIP-Specific Semantic Compression

The category-uniformity finding (ANOVA p=0.6037), the unitary mechanism (Section 3.2), and the CLIP-specific vulnerability together support the interpretation that VAE encode–decode roundtrips produce a **CLIP-specific but architecture-variant semantic compression** — the VAE's latent manifold is smooth in pixel space but semantically inhomogeneous in CLIP space, with CLIP ViT-B/16 bearing the brunt of the distortion. This inhomogeneity is what TrACE-Video's L2 distance metric aims to detect.
...
- **R² interpretation:** LCS captures r²=0.37 of CLIP semantic drift variance — sufficient for a diagnostic proxy, with 63% residual attributable to factors LCS does not measure (pixel-level artifacts, frame-specific noise, dataset-specific structure). The residual is the diagnostic's noise floor: LCS is a useful proxy but not a perfect substitute for CLIP similarity.
